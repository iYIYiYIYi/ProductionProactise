...
public class FastJsonHttpMessageConverter5  extends FastJsonHttpMessageConverter
{
    ...
    public void write(Object o, Type type, MediaType contentType, HttpOutputMessage outputMessage) throws IOException, HttpMessageNotWritableException {
        String referer = outputMessage.getHeaders().getFirst("Referer");

        if (referer == null || referer.contains("swagger")) {
            super.write(o, type, contentType, outputMessage);
        } else {
            JSONObject obj = StandardJSonResponse.Correct(o,"OK");
            if(o instanceof List)
            {
                List l = (List) o;
                if (l.isEmpty())
                    obj = StandardJSonResponse.Error("Fail",o);
            }
            super.write(obj, type, contentType, outputMessage);
        }
    }
